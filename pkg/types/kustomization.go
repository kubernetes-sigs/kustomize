...
type DataSources struct {
	...
	// RemoteKubernetesSecretSource format should be a namespace/name to a remotely accessible secret
	// on a Kubernetes cluster which access is available for, and the data keys of the precise
	// values that should be retrieved.
	//
	// e.g. if you had the following Secret object available on the remote Kubernetes cluster:
	//
	// apiVersion: v1
	// kind: Secret
	// data:
	...
	// type: Opaque
	//
	// Then you could use the following YAML to provide it as a Source:
	//
	// secretGenerator:
	// - name: "mysecret"
	...
	//   type: Opaque
	//
	// By default the current Kubernetes context and credentials available at ~/.kube/config
	// will be used to retrieve this secret, and the value set for "current-context" in that
	// configuration will be used for cluster access. Provide the `kubeconfig` field if you
	// want to use a specific configuration file or context:
	//
	// secretGenerator:
	// - name: "mysecret"
	//   remoteKubernetesSecret:
	...
	//   type: Opaque
	//
	// This allows secrets to be generated by copying some or all of an existing secret on a
	// Kubernetes cluster which access is available for, without those secrets ever having to
	// be stored on the local disk.
	RemoteKubernetesSecretSource `json:"remotekubernetessecret,inline,omitempty" yaml:"remotekubernetessecret,inline,omitempty"`
}

// RemoteKubernetesSecretSource provides access information for a secret stored on a remote
// Kubernetes cluster that should be used as a source.
type RemoteKubernetesSecretSource struct {
	// Name is the name of the object, corresponding with the provided Namespace which identifies
	// the secret object on the remote cluster which will be used
	Name string `json:"name,omitempty" yaml:"name,omitempty"`

	// Namespace is the namespace in which the secret object exists on the remote cluster
	Namespace string `json:"namespace,omitempty" yaml:"namespace,omitempty"`

	// Config is an optional path to a Kubernetes config file where the current context is
	// the valid cluster which stores the secret referenced by SelfLink and Data. If left empty
	// this will default to `~/.kube/config`.
	Config string `json:"config,omitempty" yaml:"config,omitempty"`

	// Context is an optional string containing the context within the Config that should be used
	// to access the Kubernetes cluster which houses the correct secrets for retrieval.
	// If left blank, it will default to whatever "current-context" in the configuration is set to.
	Context string `json:"context,omitempty" yaml:"context,omitempty"`

	// Data is the key within the "data" field on the secret that should be used as the value.
