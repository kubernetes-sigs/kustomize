...
func (kt *KustTarget) accumulateResources(
	...
	for _, path := range paths {
		...
		if errF := kt.accumulateFile(ra, path, origin); errF != nil {
			...
			if err == nil {
				origin.Repo = repoSpec.Host + repoSpec.OrgRepo
				absPath := repoSpec.AbsPath()
				path = absPath[strings.Index(absPath[1:], "/")+1:][1:]
				origin.Path = ""
				origin.Ref = repoSpec.Ref
			}
			origin.Path = filepath.Join(origin.Path, path)
			ra, err = kt.accumulateDirectory(ra, ldr, origin, false)
			// reset the origin so that it does not retain path information from
			// the directory accumulation
			origin = &originCopy
			if err != nil {
				return nil, errors.Wrapf(
					err, "accumulation err='%s'", errF.Error())
...
func (kt *KustTarget) accumulateDirectory(
	...
	subKt.kustomization.BuildOptions = kt.kustomization.BuildOptions
	var bytes []byte
	path := ldr.Root()
...
func (kt *KustTarget) accumulateFile(
	...
	if kt.kustomization.BuildOptions != nil && kt.kustomization.BuildOptions.AnnoOrigin {
		var anno string
		anno = anno + "path: " + filepath.Join(origin.Path, path) + "\n"
		if origin.Repo != "" {
		...
		err = resources.AnnotateAll(resource.OriginAnnotation, anno)
		if err != nil {
