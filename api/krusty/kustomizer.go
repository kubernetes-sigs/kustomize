...
func (b *Kustomizer) Run(
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to NewLoader, error:%w", err)
	}
	defer ldr.Cleanup()
	...
	err = kt.Load()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to Load, error:%w", err)
	}
	var bytes []byte
	if openApiPath, exists := kt.Kustomization().OpenAPI["path"]; exists {
		bytes, err = ldr.Load(openApiPath)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to load openApiPath, error:%w", err)
		}
	}
	err = openapi.SetSchema(kt.Kustomization().OpenAPI, bytes, true)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to SetSchema, error:%w", err)
	}
	...
	m, orderKeys, err = kt.MakeCustomizedResMap()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to MakeCustomizedResMap, error:%w", err)
	}
	...
	if b.options.AddManagedbyLabel || utils.StringSliceContains(kt.Kustomization().BuildMetadata, types.ManagedByLabelOption) {
		...
		err = t.Transform(m)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to Transform, error:%w", err)
		}
	}
