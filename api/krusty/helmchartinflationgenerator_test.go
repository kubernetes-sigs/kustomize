...
// Regression test for https://github.com/kubernetes-sigs/kustomize/issues/6077
// A Deployment generated by Helm that mounts a kustomize-generated ConfigMap and
// Secret as volumes must keep the full name-suffix-hashed names in its
// volumes[].configMap.name and volumes[].secret.secretName references after the
// namespace transformer runs. Before the fix in PR #6108, setting a namespace
// dropped the hash suffix, leaving the bare names (e.g. test-config) and breaking
// the references.
