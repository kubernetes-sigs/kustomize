...
[Kubernetes AI Guidance]: https://github.com/kubernetes/community/blob/main/contributors/guide/pull-requests.md#ai-guidance
[Kubernetes Issue Triage Guide]: https://www.kubernetes.dev/docs/guide/issue-triage/#self-assigning
[Kubernetes AI Code Review Tools Policy]: https://github.com/kubernetes/community/blob/main/github-management/ai-code-review-tools.md
[Gateway API AI Policy]: https://github.com/kubernetes-sigs/gateway-api/blob/main/AI-POLICY.md
...
Contributor and reviewer time is limited. Before opening an issue or pull request:

1. Search open and closed issues and open, closed, and merged pull requests for the same problem or change. Record the search terms you used and link the closest relevant results, if any. If there are no relevant results, state that explicitly. A similar item is not necessarily a duplicate, but you must explain any relevant difference.
1. If you intend to work on an existing issue, read its entire discussion and check its assignees. Use `/assign` before beginning implementation, as described in the [Kubernetes Issue Triage Guide]. If the command cannot assign you, leave a comment stating that you want to work on it and wait for maintainer acknowledgement.
1. If someone is already assigned, do not start work or open a competing pull request. First agree on ownership or collaboration with the assignee, or ask a maintainer to reassign the issue. Link that public agreement in your pull request.
1. Do not open a feature pull request until the issue is marked `triage/accepted` or the applicable enhancement proposal has been approved. Small documentation and maintenance changes do not always require an issue.

If you cannot perform a live search or verify assignment state, say so. Do not claim that no duplicate or assignee exists without checking.

### Use of generative AI

Kustomize follows the [Kubernetes AI Guidance]. The requirements below are additional repository-specific rules, adapted in part from the [Gateway API AI Policy]. They apply to generative AI tools, large language models, and coding agents.

#### Disclosure and accountability

- In every human-authored issue and pull request, disclose whether you used an AI tool while preparing the contribution. If you did, name the provider, product, model, and version. Use `unknown/auto-selected` when the tool does not expose the model, and use `None` when no AI tool was used. Keep the disclosure factual; it is provenance, not a product endorsement.
- Briefly explain how the tool was used. This includes assistance with code, tests, documentation, research, summarization, copy-editing, or translation. Do not paste prompts, chat transcripts, or chain-of-thought.
- You remain responsible for correctness, security, licensing, clarity, and compliance. You must understand and be able to explain every submitted change.
- Trusted project automation that maintainers configure and identify may use `N/A` for contributor disclosures, attestations, assignment, and Draft requirements. It remains subject to scope, correctness, duplicate, and review-cost checks. A third-party bot is not exempt merely because it is automated. Adding an automated AI review tool requires the process in the [Kubernetes AI Code Review Tools Policy].

#### Human-authored project communication

Do not post project communication whose substantive content was generated by an AI tool. The facts, reasoning, decisions, and responses in issue and pull request descriptions, comments, reviews, GitHub Discussions, project Slack messages, and shared design documents or discussions must be written and owned by the person posting them.

Non-generative assistive technology is outside the scope of this policy. You may use generative AI for light spelling, grammar, accessibility, or translation assistance on text that you wrote when it does not introduce or materially change facts, reasoning, decisions, or requested actions. Review the final meaning and disclose the assistance.

After review feedback is received, do not use an AI tool to analyze or summarize the feedback, decide how to address it, implement the response, or draft, rewrite, or translate a reply. The human author must interpret and address the feedback and engage directly with the reviewer, as required by the [Kubernetes AI Guidance].

Relevant non-generative tool output, such as compiler or test output, may be quoted as evidence. Output necessary to report a defect in an AI integration must be clearly identified, kept to the necessary excerpt, and accompanied by the author's own explanation.

This communication restriction governs contributor-authored communication. It does not prohibit project-approved automation enabled under the [Kubernetes AI Code Review Tools Policy], which is governed by its approved configuration and oversight.

Do not infer, publicly question, or accuse someone of AI use based on writing style or other proxies. If a required disclosure field is missing, ask only that it be completed. Raise any other concern supported by objective evidence privately with maintainers.

#### Concise and reviewable pull request descriptions

A pull request marked ready for review must be concise but complete:

- Summarize the change in no more than three focused bullets and explain why it is needed in one short paragraph.
- Link to the accepted issue or design document instead of repeating its background, alternatives, or discussion history.
- Do not narrate the diff file by file, repeat information visible in the diff, or include generated walkthroughs, prompts, chat transcripts, or raw test logs.
- Keep reproduction steps, compatibility or security considerations, test commands and results, release-note information, and the required AI disclosure as detailed as necessary. For large output, report the command, outcome, and relevant failure excerpt; link a durable artifact when one is available. Do not paste complete raw logs solely to satisfy the validation requirement.

Draft pull requests may contain temporary working notes, but they must meet these requirements before being marked ready for review. Reviewers may ask for an unnecessarily long or repetitive description to be rewritten before review continues.

#### AI-assisted changes and review readiness

- Large AI-generated pull requests and AI-generated commit messages are not allowed. Keep changes small and coherent; do not split a change merely to evade review.
- Do not list an AI tool as an author or co-author, co-sign with an AI identity, or add `assisted-by`, `co-developed-by`, `co-authored-by`, or similar AI attribution trailers.
- A Draft pull request is not a substitute for the self-review required before submission. Before opening any pull request, including a Draft, the human author must review every submitted line, verify the reported validation results, and understand and be able to explain the change.
- When generative AI produced or materially rewrote code, tests, repository documentation, or substantive analysis in the proposed diff, initially open the pull request as a GitHub Draft. This preserves the required conventional pull request title; use `/hold` as a fallback when Draft state is unavailable. Private research or light copy-editing or translation of a human-written description alone does not trigger the Draft requirement.
- Only the human author may mark the pull request ready after reviewing the final diff and validation results and writing the final description independently. A coding agent must not make this attestation or perform the Draft-to-ready transition.

`Ready for review` only means that author self-review is complete. It does not mean that the pull request is ready to merge; normal reviewer approval and merge requirements still apply.

Maintainers may use the [private AI contribution triage prompt](.github/prompts/ai-contribution-triage.md) to gather advisory findings. They must verify every finding and write any public response themselves; prompt output must not be posted directly or used for automatic enforcement.
...
### Build Kustomize
The [Kustomize Architecture] document describes the repository organization and the kustomize build process. Run these commands from the repository root.
```bash
# For go version >= 1.13
unset GOPATH
unset GO111MODULES
